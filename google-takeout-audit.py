...
def finalize_search_regrets(regrets: list[dict[str, Any]]) -> SearchStats:
...
def detect_account(root: Path) -> str:
    pattern = re.compile(r"Archive for\s+([^\s<]+@[^\s<]+)", re.I)
    for path in root.rglob("archive_browser.html"):
        try:
            match = pattern.search(path.read_text(encoding="utf-8", errors="replace")[:200_000])
        except OSError:
            continue
        if match:
...
<div class="hint">💡 Ouvre via <code>takeout-audit.sh</code> ou <code>python3 -m http.server</code> dans ce dossier — <code>xdg-open file://</code> bloque souvent les graphiques.</div>
<div id="error"></div>
...
<div class="grid">
  ...
  <div class="panel" id="regretPanel">
    ...
    <p style="color:#a9b1d6;font-size:.9rem">Mix diversifié (rant, vocal, santé, relation, légal…). Filtre par tag ci-dessous.</p>
...
def main() -> int:
    ...
    log("  Bonne méthode: ./takeout-audit.sh <dossier-takeout>")
